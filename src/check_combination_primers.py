...
"""
USAGE
    ./check_combination_primers.py [-h] <fastq_file> <cutadapt_info_file> <output_file> <output_file2> [-r <yes|no>] [-i <output_log>] [-c <output_counts>]
DESCRIPTION
    check_combination_primers.py is a script to keep sequences where the correct combinations of primers have been found.
PREREQUISITE
    - python3
    - Cutadapt info-file
INFORMATION
...
def parse_cutadapt_log(cutadapt_file):
    """
    Parse input Cutadapt info-file and keep seqId of sequences where primers was find into a dict like : {seqId : [primers]}
    and seqId of sequences where wrong primers was find into a dict like : {"without":[seqId], "one":[seqId], "wrong":[seqId]}
    """
...
def write_output_files(fastqDict, seqGoodPrimersDict, seqWrongPrimersDict, output_file, output_file2, reverse_complement, output_log, output_counts):
    """
    Write the new FASTQ file with sequences saved, a FASTQ file with sequences unsaved and log files : 1/ with id of sequences not saved, 2/ with counts of sequences not saved
    """
    ...
    for typeErrorPrimer in seqWrongPrimersDict.keys():
        for seqId in seqWrongPrimersDict[typeErrorPrimer]:
            if (len(fastqDict[seqId]["seq"]) != 0):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="check_combination_primers.py is a script to keep sequences where the correct combinations of primers have been found.")
    ...
    write_output_files(file_dict, seq_good_primers, seq_wrong_primers, args.output_file, args.output_file2, args.reverse_complement, args.output_log, args.output_counts)